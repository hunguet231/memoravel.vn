...
export const checkUpdateProduct = async (req, res, next) => {
  try {
    ...
    const productIdFromDatabase = products?.map(({ id }) => id) || [];
    const isExistProduct = productIdFromDatabase.includes(productId);

    if (!isExistProduct) {
      return res
        .status(AppConst.STATUS_BAD_REQUEST)
        .json(responseFormat({ message: 'Sản phẩm này không tồn tại' }));
    }
